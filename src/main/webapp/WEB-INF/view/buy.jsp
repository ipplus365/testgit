<%@ page language="java" contentType="text/html; charset=UTF-8"  pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
	<head>
		<meta charset="UTF-8">
		<title>商品详情</title>
		<link rel="stylesheet" type="text/css" href="/statics/css/reset.css"/>
		<link rel="stylesheet" type="text/css" href="/statics/css/iconfont.css"/>
		<link rel="stylesheet" type="text/css" href="/statics/css/nav.css"/>
		<link rel="stylesheet" type="text/css" href="/statics/css/footer.css"/>
		<link rel="stylesheet" type="text/css" href="/statics/css/buy.css"/>
		<link rel="shortcut icon" href="/statics/images/favicon.ico"/>
	</head>
	<body>
		<!--导航栏开始-->
		<div class="nav">
			<div class="nav_inner">
				<div class="nav_inner_left">
					<li>
						<a href="http://www.ipplus360.com/">首页</a>
					</li>
					<li>
						<a href="javascript:void(0)">产品中心</a>
						<div class="list">
							<a href="http://www.ipplus360.com/pros/iph/">IP问问-高精准</a>
							<a href="http://www.ipplus360.com/pros/ipm/">IP问问-区县级</a>
							<a href="http://www.ipplus360.com/pros/ips/">IP Scene(应用场景)</a>
						</div>
					</li>
					<li>
						<a href="javascript:void(0)">应用领域</a>
						<div class="list">
							<a href="http://www.ipplus360.com/apparea/ad/">互联网广告投放</a>
							<a href="http://www.ipplus360.com/apparea/anti-cheating/">互联网广告反作弊</a>
							<a href="http://www.ipplus360.com/apparea/anti-fraud/">反欺诈风控</a>
							<a href="http://www.ipplus360.com/apparea/bigdata/">位置大数据分析</a>
						</div>
					</li>
					<li>
						<a href="javascript:void(0)">技术支持</a>
						<div class="list">
							<a href="http://www.ipplus360.com/tech/faq/">常见问题</a>
							<a href="http://www.ipplus360.com/tech/baike/">技术文档</a>
							<a href="http://www.ipplus360.com/tech/api/">API实例</a>
						</div>
					</li>
					<li>
						<a href="javascript:void(0)">公司介绍</a>
						<div class="list">
							<a href="http://www.ipplus360.com/about/us/">埃文介绍</a>
							<a href="http://www.ipplus360.com/about/join/">加入我们</a>
							<a href="http://www.ipplus360.com/news/">新闻资讯</a>
						</div>
					</li>
					<li>
						<a href="Javascript:;">实用工具<img src="/statics/images/search-red.png"/></a>
						<div class="list">
							<a href="http://www.ipplus360.com/ip/">IP查询</a>
							<a href="http://www.ipplus360.com/tools/">IP Tools</a>
						</div>
					</li>
				</div>
				<div class="nav_inner_right">
					<li>
						<a href="http://www.ipplus360.com/tech/faq/">帮助与支持</a>
					</li>
					<li class="login_box">
						<!--<a href="javascript:void(0)" id="login">登录</a>-->
					</li>
				</div>
			</div>
		</div>
		<!--导航栏结束-->
		<div id="main">
			<div class="main_inner">
				<div class="left">
					<li class="one active">
						<img src="/statics/images/t_s_title1_white.png"/>
						IP问问—高精准
					</li>
					<li class="two">
						<img src="/statics/images/t_s_title2.png"/>
						IP问问—区县级
					</li>
					<li class="three">
						<img src="/statics/images/t_s_title3.png"/>
						IP Scene
					</li>
				</div>
				<!--购买页面街道级 开始-->
				<div class="right street">
					<!--<form action="" method="post" class="right street">
						<input type="hidden" name="pid" id="" value=""/>
						<input type="hidden" name="" id="" value=""/>
						<input type="hidden" name="" id="" value=""/>
						<input type="hidden" name="" id="" value=""/>-->
						<li>
							<span class="black bold mr50">产品名称</span>
							<span class="size16 bold black1 spacing">IP问问—高精准</span>
						</li>
						<li>
							<span class="black bold mr50">定位精度</span>
							街道级
						</li>
						<li>
							<span class="black bold mr50">计费单价</span>
							<span class="normal size14 light">¥<em class="orange size18 bold streetPrice"></em>/千次</span>
							<span class="normal light ml50 size14 streetTips">小额流量包限购三次，一年有效期</span>
						</li>
						<li>
							<span class="black bold mr50">选择规格</span>
							<ul class="street_model_list">
								<!--<li class="active">10万</li>
								<li>500万</li>
								<li>5000万</li>
								<li>1亿</li>
								<li>5亿</li>
								<li class="last">10亿</li>-->
							</ul>
						</li>
						<li>
							<span class="black bold mr50">选择数量</span>
							<div class="num">
								<span class="reduce">-</span><input type="text" name="" id="number" value="1" readonly="" maxlength="1000"/><span class="add">+</span>
							</div>
							<span class="light ml50 size14 normal">*购买数量多于1000个，请联系客服</span>
						</li>
						<li>
							<span class="black bold mr50">配置信息</span>
							<table border="" cellspacing="0" cellpadding="0">
								<tr>
									<td class="bg" >覆盖范围</td>
									<td width="600px">
										1.街道级别、区县级别精度下的服务可覆盖中国大陆地区（境外地区已开通部分服务，如需境外<br>地区高精度IP定位服务，请联系公司）；<br>
										2.城市级别精度下的服务可覆盖全球；<br>
										3.本产品为IPV4全库。
									</td>
								</tr>
								<tr>
									<td class="bg">更新频率</td>
									<td>
										数据动态更新，全库数据将采用以下频率更新：<br>
										1.每月更新；<br>
										2.每周更新（即将推出）。
									</td>
								</tr>
								<tr>
									<td class="bg">产品特点</td>
									<td>1.最高可达街道级别精度；<br>
										2.多区域划分IP定位区域；<br>
										3.提供定位准确度、最大误差距离、稳定性等性能指标。<br>
									</td>
								</tr>
								<tr>
									<td class="bg">在线并发</td>
									<td>1.每秒1000次查询；<br />2.10ms返回场景结果。</td>
								</tr>
								<tr>
									<td class="bg">注意事项</td>
									<td>用户应用该产品时需要符合国家法律规定，如有危害国家安全行为，<br />用户须为自己的行为负全部责任。</td>
								</tr>
								<tr>
									<td class="bg">优惠规则</td>
									<td>大额度流量包（1亿及其以上），一次性连续购买享受优惠折扣。<br />
										一次性连续购买6个流量包可享9.5折<br />
										一次性连续购买12个流量包可享9.0折<br />
										一次性连续购买24个流量包可享8.5折<br />
										一次性连续购买36个流量包可享8.0折；
									</td>
								</tr>
							</table>
						</li>
						<li>
							<span class="normal black">价格：</span>
							<em class="orange normal">¥<b class="size28 orange bold streetTotal"></b></em>
							<div class="btnDiv">
								<button class="addCart">加入购物车</button>
								<!--<a href="settle_accounts.html" class="pay">点击付款</a>-->
								<!--<input type="submit" value="点击付款" class="pay"/>-->
							</div>
						</li>
					<!--</form>-->
				</div>
				<!--购买页面街道级 结束-->
				<!--购买页面区县库部分 开始-->
				<div class="right district">
					<li>
						<span class="black bold mr50">产品名称</span>
						<span class="size16 bold black1 spacing">IP问问—区县级</span>
					</li>
					<li class="no_padding district_API">
						<h2>IP问问-区县级API<i class="iconfont">&#xe7de;</i></h2>
					</li>
					<div class="API_slide">
						<li>
							<span class="black mr50 bold">配置信息</span>
							<table border="" cellspacing="0" cellpadding="0">
								<tr>
									<td class="bg">定位精度</td>
									<td width="600px">洲级/国家级/省级/城市级/区县级</td>
								</tr>
								<tr>
									<td class="bg">产品性能</td>
									<td>在线并发：1000次/s</td>
								</tr>
								<tr>
									<td class="bg">覆盖区域</td>
									<td>区县精度级别覆盖中国大陆地区（不含香港、台湾地区） <br />城市精度级别覆盖全球<br /> 产品数据为IPV4全库 </td>
								</tr>
								<tr>
									<td class="bg">提供内容</td>
									<td>经纬度<br />组织机构<br />洲、国家、省、市、区县的行政区信息  </td>
								</tr>
							</table>
						</li>
						<li>
							<span class="black bold mr50">计费单价</span>
							<span class="orange bold size18 mr50 district_api_single">1920.00元/年</span>
							<span class="normal light size14 free_tips" style="display: none;">*2017年12月1日优惠结束</span>
						</li>
						<li>
							<span class="black bold mr50">API规格</span>
							<ul class="API_list district_API_amout">
								<li districtApiPriceId="7" class="active">3万次/日 <span>1920元/年<b></b></span> </li>
								<li districtApiPriceId="8">80万次/日<span>9600元/年<b></b></span></li>
								<li districtApiPriceId="9">150万次/日<span>18000元/年<b></b></span></li>
								<li districtApiPriceId="10">300万次/日<span>36000元/年<b></b></span></li>
								<li districtApiPriceId="50" class="last">5000次<span class="district_free">限时免费<b></b></span></li>
							</ul>
						</li>
						<li>
							<span class="black bold mr50">选择时间</span>
							<ul class="time_list">
								<li districtApiYear='1' class="active">1年</li>
								<li districtApiYear='2'>2年</li>
								<li districtApiYear='3' class="last">3年</li>
							</ul>
							<span class="light size14 ml50">*一次性购买三年享受9.0折优惠，如果需要请联系客服</span>
						</li>
					</div>
					<li class="no_padding district_download">
						<h2>IP问问-区县库下载<i class="iconfont">&#xe6ca;</i></h2>
					</li>
					<div class="API_download">
					<!-- 新增 -->
						<li class="API_download_configure">
							<span class="configure_title">配置信息</span>
							<table border="" cellspacing="0" cellpadding="0">
								<tbody><tr>
									<td class="bg">覆盖范围</td>
									<td width="600px">
										1.区县级别精度服务可覆盖中国大陆地区（不含香港、台湾地区） <br>
										2.本产品为IPV4全库，可覆盖全球所有地区。
									</td>
								</tr>
								<tr>
									<td class="bg">应用领域</td>
									<td>
										1.基于位置服务的在线广告精准投放； <br>
										2.网络安全风险控制。
									</td>
								</tr>
								<tr>
									<td class="bg">注意事项</td>
									<td>
										用户应用该产品时需要符合国家法律规定，如有危害国家安全行为，<br>
										用户须为自己的行为负全部责任。 
									</td>
								</tr>
								<tr>
									<td class="bg">优惠规则</td>
									<td>
										区县级IP定位产品可免费试用。
									</td>
								</tr>
							</tbody></table>
						</li>
						<!-- 新增end -->
						<li>
							<span class="black bold mr50">使用方式</span>
							<span class="black normal">离线数据库</span>
						</li>
						<li class="no_border">
							<span class="black bold block">版本选择</span>
							<div class="version_right">
								<ul class="version district_version">
									<li versionId="3" class="active">当前版本</li>
									<li versionId="4" class="last">历史版本</li>
								</ul>
							</div>
						</li>
						<li class="no_border">
							<span class="black bold block">坐标系选择</span>
							<div class="coordinate_right">
								<ul class="coordinate">
									<li coordId='1' class="active">GPS坐标系</li>
									<li coordId='2' class="last">百度坐标系</li>
								</ul>
								<span class="light size14 spacing ml50">*GPS坐标系（WGS84），百度坐标系（BD09）</span>
							</div>
						</li>
						<li class="choose district_now">
							<span class="normal midGray block">更新频率</span>
							<ul class="version auto_width">
								<li class="active last">月</li>
							</ul>
							<br />
							<br />
							<span class="normal midGray block">数据格式</span>
							<ul class="version auto_width">
								<li class="active">MySQL</li>
							</ul>
							<span class="normal size14 light ml50">*每用户每日限下载三次/每月更新</span>
							<br />
							<br />
							<span class="normal midGray block">区域</span>
							<ul class="version auto_width">
								<li class="active">多区域</li>
							</ul>
							<br />
							<br />
							<span class="normal midGray block">更新方式</span>
							<ul class="version auto_width">
								<li class="active">全量更新</li>
							</ul>
						</li>
						<li class="choose district_history">
							<span class="bold black block">数据版本</span>
							<ul class="version auto_width">
								<li class="active">6月</li>
								<li class="active">7月</li>
								<li class="active">8月</li>
							</ul>
						</li>
					</div>
					<li class="district_api_pay">
						<span class="normal black">价格：</span>
						<em class="normal orange">¥<b class="bold orange size28 district_api_total">1920.00</b></em>
						<div class="btnDiv">
							<button class="addCart">加入购物车</button>
							<!--<button class="pay">点击付款</button>-->
						</div>
					</li>
					<li class="district_download_pay">
						<span class="normal black">价格：</span>
						<em class="normal orange">¥<b class="bold orange size28 district_download_total">0.00</b></em>
						<span class="black midGray normal" style="text-decoration: line-through;">¥53000</span>
						<div class="btnDiv">
		<!-- 					<a href="/info" class="try_free">申请免费</a> -->
							<button class="addCart">加入购物车</button>
							<!--<button class="pay">点击付款</button>-->
						</div>
						<!-- <p class="warning"><img src="/statics/images/abandon.png"/>只有经过认证的企业用户才可免费使用</p> -->
					</li>
					<li class="district_download2_pay">
						<span class="normal black">价格：</span>
						<em class="normal orange">¥<b class="bold orange size28">9680.00</b></em>
						<span class="black midGray normal" style="text-decoration: line-through;">¥40000</span>
						<div class="btnDiv">
							<button class="addCart">加入购物车</button>
							<!--<button class="pay">点击付款</button>-->
						</div>
					</li>
				</div>
				<!--购买页面区县级 结束-->
				<!--购买页面ip场景 开始-->
				<div class="right scene">
					<li>
						<span class="black bold mr50">产品名称</span>
						<span class="black1 bold size16 spacing">IP Scene （应用场景）</span>
					</li>
					<li class="no_padding scene_API">
						<h2>IP Scene （应用场景）&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;API<i class="iconfont">&#xe6ca;</i></h2>
					</li>
					<div class="API_slide">
						<li>
							<span class="black mr50 bold">配置信息</span>
							<table border="" cellspacing="0" cellpadding="0">
								<tr>
									<td class="bg" >覆盖范围</td>
									<td width="600px">此版本产品覆盖中国大陆地区（如需境外地区数据请与公司联系）  </td>
								</tr>
								<tr>
									<td class="bg">应用领域</td>
									<td>1.在线广告精准投放过程中过滤机器人爬虫流量；<br />2.网络安全风险控制。</td>
								</tr>
								<tr>
									<td class="bg">产品特点</td>
									<td>最高可达14种应用场景，为用户行为分析提供帮助；</td>
								</tr>
								<tr>
									<td class="bg">在线并发</td>
									<td>1.每秒1000次查询；<br />2.10ms返回场景结果。</td>
								</tr>
								<tr>
									<td class="bg">注意事项</td>
									<td>用户应用该产品时需要符合国家法律规定，如有危害国家安全行为，<br />用户须为自己的行为负全部责任。</td>
								</tr>
								<tr>
									<td class="bg">优惠规则</td>
									<td>大额度流量包（500万及其以上），一次性连续购买享受优惠折扣。<br />
										一次性连续购买6个流量包可享9.5折<br />
										一次性连续购买12个流量包可享9.0折<br />
										一次性连续购买24个流量包可享8.5折<br />
										一次性连续购买36个流量包可享8.0折；
									</td>
								</tr>
							</table>
						</li>
						<li>
							<span class="black bold mr50">提供场景</span>
							<ul class="scene_provide" style="width: 560px;">
								<li class="active">住宅用户</li>
								<li class="active">企业专线</li>
								<li class="active">组织机构</li>
								<li class="active">数据中心</li>
								<li class="active">学校单位</li>
								<li class="active">基础设施</li>
								<li class="last active">未分配</li>
								<li class="no_top active">移动网络</li>
								<li class="no_top active">WLAN热点</li>
								<li class="no_top active">卫星通信</li>
								<li class="no_top active">交换中心</li>
								<li class="no_top size12 active">已路由-未使用</li>
								<li class="no_top size12 active">已分配-未路由</li>
								<li class="no_top last active">保留IP段</li>
							</ul>
						</li>
						<li>
							<span class="black bold mr50">选择规格</span>
							<ul class="scene_model_list">
								<li class="active">500万</li>
								<li>1000万</li>
								<li>1亿</li>
								<li>5亿</li>
								<li class="last">10亿</li>
							</ul>
						</li>
						<li>
							<span class="black bold mr50">计费单价</span>
							<span class="normal size14 light">¥<em class="orange size18 bold scenePrice"></em>元/次</span>
							<span class="normal midGray ml50 size14">大额度流量包（500万及其以上），一次性连续购买享受优惠折扣</span>
						</li>
						<li>
							<span class="black bold mr50">选择数量</span>
							<div class="num">
								<span class="reduce">-</span><input type="text" name="" id="sceneNumber" value="1" readonly=""/><span class="add">+</span>
							</div>
							<span class="light size14 ml50">*购买数量多于1000个，请联系客服</span>
						</li>
					</div>
					<!--<li class="no_padding scene_download">
						<h2>场景库下载<i class="iconfont">&#xe6ca;</i></h2>
					</li>
					<div class="API_download">
						<li class="no_border">
							<span class="black block bold">选择场景</span>
							<div class="scenechoose_right">
								<ul class="API_list scene_download_single_list">
									<li class="active">住宅用户 <span>30000元/年<b></b></span> </li>
									<li>企业专线<span>41000/年<b></b></span></li>
									<li>组织结构</li>
									<li>数据中心<span>32000元/年<b></b></span></li>
									<li>学校单位<span>9000元/年<b></b></span></li>
									<li>移动网络<span>4200元/年<b></b></span></li>
									<li class="last">WLAN</li>
								</ul>
								<ul class="API_list scene_download_more_list" style="margin-top: 50px;">
									<li>基础设施 <span>8400元/年<b></b></span> </li>
									<li>交换中心</li>
									<li>卫星通信</li>
									<li>未分配</li>
									<li style="width: 125px;">已路由-未使用</li>
									<li class="last" style="width: 125px;">已分配-未路由</li>
								</ul>
								<ul class="API_list" style="margin-top: 50px;display: block;">
									<li class="active last">保留IP段<span class="gift">赠</span></li>
								</ul>
							</div>
						</li>
						<li>
							<span class="black bold size16 block">选择时间</span>
							<ul>
								<li class="active">1年</li>
								<li>2年</li>
								<li class="last">3年</li>
							</ul>
						</li>
					</div>-->
					<li class="scene_api_pay">
						<strong class="paySpan black">价格：</strong>
						<em class="normal orange">¥<b class="bold orange size28 sceneTotal"></b></em>
						<span class="black black normal" style="text-decoration: line-through;"></span>
						<div class="btnDiv">
							<button class="addCart">加入购物车</button>
							<!--<button class="pay">立即购买</button>-->
						</div>
					</li>
					<!--<li class="scene_download_pay">
						<strong class="paySpan black">支付价格：</strong>
						<em class="normal orange">¥<b class="bold orange size28 sceneDownloadTotal">30000</b></em>
						<span class="black black normal" style="text-decoration: line-through;">¥90000</span>
						<div class="btnDiv">
							<button class="addCart">加入购物车</button>
							<button class="pay">立即购买</button>
						</div>
					</li>-->
				</div>
				<!--购买页面ip场景 结束-->
			</div>
			<!-- 蓝色购物车按钮 部分 -->
			<div class="cart_button">
				<div class="inner">
					<i class="iconfont">&#xe600;</i>
					购物车
					<span></span>
				</div>
			</div>
			<!-- 购物车详细内容部分 -->
			<div class="cart">
				<h2><i class="iconfont">&#xe600;</i>购物车<span>></span></h2>
				<div class="content">
					<!--<li>
						<h2>IP问问-高精准<i class="iconfont">&#xe618;</i></h2>
						<p>购买规格：十万</p>
						<p>购买数量：2个</p>
						<p>计费单价：¥99.00/十万</p>
						<p>计费价格：¥198.00</p>
						<p>支付价格：<em>¥198.00</em></p>
					</li>
					<li>
						<h2>IP问问-区县级<i class="iconfont">&#xe618;</i></h2>
						<p>购买规格：十万</p>
						<p>购买数量：2个</p>
						<p>计费单价：¥99.00/十万</p>
						<p>计费价格：¥198.00</p>
						<p>支付价格：<em>¥198.00</em></p>
					</li>-->
					<li class="null">
						购物车空空的，快去选购吧
					</li>
				</div>
				<div class="cart_pay">
					<p>支付总额：<em>¥0.00</em></p>
					<a href="/cart/checkout">立即购买</a>
				</div>
			</div>
		</div>
	
		<div class="footer">
    		<div class="footer-inner">
    			<div class="footer-top">
    				<!--<a href="https://mall.ipplus360.com/" target="_blank">注册有礼</a>
    				<a href="https://mall.ipplus360.com/" target="_blank" class="two">免费试用</a>-->
    			</div>
    			<div class="footer-mid">
				    <ul>
				   		<li class="first">产品中心</li>
				   		<li class="other two"><a href="http://www.ipplus360.com/pros/iph" >IP问问-高精准</a></li>
                  		<li class="other two"><a href="http://www.ipplus360.com/pros/ipm" >IP问问-区县级</a></li>
                  		<li class="other two"><a href="http://www.ipplus360.com/pros/ips" >IP Scene(应用场景)</a></li>
                	</ul>
                	<ul>
	                	<li class="first">应用领域</li>
	                	<li class="other two"><a href="http://www.ipplus360.com/apparea/ad" >互联网广告投放</a></li>
	     				<li class="other two"><a href="http://www.ipplus360.com/apparea/anti-cheating" >互联网广告反作弊</a></li>
	                	<li class="other two"><a href="http://www.ipplus360.com/apparea/anti-fraud" >反欺诈风控</a></li>
	                  	<li class="other two"><a href="http://www.ipplus360.com/apparea/bigdata" >位置大数据分析</a></li>
                	</ul>
                	<ul>
						<li class="first">技术支持</li>
		  				<li class="other two"><a href="http://www.ipplus360.com/tech/faq" >常见问题</a></li>
		                <li class="other two"><a href="http://www.ipplus360.com/tech/baike" >技术文档</a></li>
		                <li class="other two"><a href="http://www.ipplus360.com/tech/api" >API示例</a></li>
                	</ul>
                	<ul>
	                	<li class="first">公司介绍</li>
	                	<li class="other two"><a href="http://www.ipplus360.com/about/us" >埃文介绍</a></li>
	                 	<li class="other two"><a href="http://www.ipplus360.com/about/join" >加入我们</a></li>
	                 	<li class="other two"><a href="http://www.ipplus360.com/news" >新闻资讯</a></li>
                	</ul>
                	<ul>
	                	<li class="first">实用工具</li>
	                	<li class="other two"><a href="http://www.ipplus360.com/ip/" >IP查询</a></li>
	                  	<li class="other two"><a href="http://www.ipplus360.com/tools/" >IP Tools</a></li>
                	</ul>
					<ul class="five">
    					<li class="first">联系我们</li>
    					<li class="other two"><a href="javascript:;"><i class="iconfont">&#xe71b;</i>contacts@ipplus360.com</a></li>
    					<li class="other"><a href="javascript:;"><i class="iconfont">&#xe7dd;</i>400-0371-911</a></li>
    					<li class="other"><a href="javascript:;"><i class="iconfont">&#xe637;</i>@埃文科技IPPLUS</a></li>
    					<li class="other"><a href="javascript:;"><i class="iconfont">&#xe63a;</i>2415968308</a></li>
    					<li class="last"><a href="javascript:;">（工作日 08：30-18：00）</a></li>
    				</ul>
    				<ul class="six">
    					<i class="iconfont">&#xe659;</i>
    					<img src="/statics/images/weixin.jpg"/>
    				</ul>
    			</div>
    			<div class="footer-bot">
    				© 2012-2017 ipplus360.com , All Rights Reserved.<a href="http://www.miitbeian.gov.cn/" target="_blank" style="color:#808488;">豫ICP备12022601号-1</a>| 豫公网安备 11010802021761
    			</div>
    		</div>
    	</div>
    	<!--登录模块-->
    	<div id="pop" style="display: none;">
    		<h2>
    			<span style="color:#7e7e7e;font-size: 22px;">登录</span>
    			<span class="two">还没有账号？点击<a id="register" href="javascript:;">立即注册</a></span>
    		</h2>
    		<input type="text" name="" id="name" value="" placeholder="账户"/>
    		<input type="password" name="" id="pwd" value="" placeholder="密码"/>
    		
    		<div class="yzm">
    			<span> 
    				<img style="width: 100%;height: 100%;cursor: pointer;" src="/captcha" class="login_yzm" onclick="this.src='/captcha?d='+new Date().getTime()"/>
    			</span>
    			<input type="text" name="" id="yzm" value="" placeholder="请输入验证码"/>
    		</div>
    		<p class="regError login_error">
    			<!--<i class="i-error"></i>
    			账号密码错误-->
    		</p>
    		<button class="login">立即登录</button>
    		<p style=""><a href="/user/forget">忘记密码</a></p>
    	</div>
    	<!--注册模块-->
    	<div id="reg" style="display: none;">
    		<h2>
    			<span class="active">个人账户<i></i></span><span>企业账户<i></i></span>
    		</h2>
    		<div class="box">
    			<div class="person">
    				<p>个人注册：可免费申请IP问问区县级别数据库 </p>
    				<!--<li class="mt32">
    					<span>昵称</span>
    					<input type="text" placeholder="设置后不可更改。最长五个汉字或十个英文" class="per_name"/>
    				</li>
    				<p class="error per_name_error">
    					<i class="i-error"></i>
    					请您输入昵称
    				</p>-->
    				<li class="mt32">
    					<span>注册邮箱</span>
    					<input type="text" placeholder="请输入个人邮箱账号" class="per_email"/>
    					<i class="i-success"></i>
    				</li>
    				<p class="regError per_email_error">
    					<!--<i class="i-error"></i>
    					请您输入邮箱-->
    				</p>
    				<li class="">
    					<span>登录密码</span>
    					<input type="password" placeholder="6-20位，不能出现空格" id="regPwd" class="per_pwd"/>
    					<i class="i-success"></i>
    				</li>
    				<p class="regError per_pwd_error">
    					<!--<i class="i-error"></i>
    					请您输入密码-->
    				</p>
    				<li class="">
    					<span>重复密码</span>
    					<input type="password" id="repeatPwd" placeholder="重复密码" class="per_repeatPwd"/>
    					<i class="i-success"></i>
    				</li>
    				<p class="regError per_repeatPwd_error">
    					<!--<i class="i-error"></i>
    					请您再次输入密码-->
    				</p>
    				<li>
    					<span>手机号码</span>
    					<input type="tel" id="repeatPwd" placeholder="请输入有效的手机号码" class="per_mobile"/>
    					<i class="i-success"></i>
    				</li>
    				<li class="spec">
    					<button class="getMobile">获取验证码</button>
    					<input type="text" name="" id="" value="" placeholder="请输入验证码" class="per_mobileCode"/>
    				</li>
    				<p class="regError per_mobileCode_error">
    					<!--<i class="i-error"></i>
    					手机验证码错误-->
    				</p>
    				<li class="mobileyzm">
    					<span><img style="width: 100%;height: 100%;cursor: pointer;" src="/captcha" onclick="this.src='/captcha?d='+new Date().getTime()"/></span>
    					<input type="text" placeholder="请输入验证码" class="per_captcha"/>
    				</li>
    				<p class="regError per_captcha_error">
    					<!--<i class="i-error"></i>
    					验证码错误-->
    				</p>
    				<button class="per_register">注册</button>
    			</div>
    			<div class="company">
    				<p>企业注册：专享IP Scene（应用场景）服务</p>
    				<!--<li>
    					<span>昵称</span>
    					<input type="text" placeholder="设置后不可更改。最长五个汉字或十个英文"/>
    				</li>-->
    				<li class="mt32">
    					<span>注册邮箱</span>
    					<input type="text" placeholder="请输入个人邮箱账号" class="com_email"/>
    					<i class="i-success"></i>
    				</li>
    				<p class="regError com_email_error">
    					
    				</p>
    				<li>
    					<span>登录密码</span>
    					<input type="password" placeholder="6-20位，不能出现空格" id="regPwdCom" class="com_pwd"/>
    					<i class="i-success"></i>
    				</li>
    				<p class="regError com_pwd_error">
    					
    				</p>
    				<li>
    					<span>重复密码</span>
    					<input type="password" id="repeatPwdCom" placeholder="重复密码" class="com_repeatPwd"/>
    					<i class="i-success"></i>
    				</li>
    				<p class="regError com_repeatPwd_error">
    					
    				</p>
    				<li>
    					<span>企业名称</span>
    					<input type="text"  placeholder="请输入公司名称" class="com_orgname"/>
    					<i class="i-success"></i>
    				</li>
    				<p class="regError com_orgname_error">
    					
    				</p>
    				<li>
    					<span>联系人</span>
    					<input type="text"  placeholder="请输入联系人" class="com_contact"/>
    					<i class="i-success"></i>
    				</li>
    				<p class="regError com_contact_error">
    					
    				</p>
    				<li>
    					<span>手机号码</span>
    					<input type="tel"  placeholder="请输入有效的手机号码" class="com_mobile"/>
    					<i class="i-success"></i>
    				</li>
    				<li class="spec">
    					<button class="getComMobile">获取验证码</button>
    					<input type="text" name="" id="" value="" placeholder="请输入验证码" class="com_mobileCode"/>
    				</li>
    				<p class="regError com_mobileCode_error">
    					
    				</p>
    				<li class="shoujiyzm">
    					<span><img style="width: 100%;height: 100%;cursor: pointer;" src="/captcha" class="com_yzm" onclick="this.src='/captcha?d='+new Date().getTime()"/></span>
    					<input type="text" placeholder="请输入验证码" class="com_captcha"/>
    				</li>
    				<p class="regError com_captcha_error">
    					
    				</p>
    				<button class="com_register">注册</button>
    			</div>
    		</div>
    		<h3>注册即表示同意我们的<a href="/user/protocol" target="_blank">服务条款</a></h3>
    		<h4>已经拥有账户<a href="javascript:;">登录</a></h4>
    		
    	</div>
	</body>
</html>
<script src="/statics/js/jquery-1.10.1.min.js" type="text/javascript" charset="utf-8"></script>
<script src="/statics/layer/layer.js" type="text/javascript" charset="utf-8"></script>
<script src="/statics/js/placeholder.js" type="text/javascript" charset="utf-8"></script>
<script src="/statics/js/login.js" type="text/javascript" charset="utf-8"></script>
<script src="/statics/js/buy.js"></script>


